...
export async function GET() {
  ...
  try {
    ...
    // Aggregate: same payor + same ATC + same quarter → one line
    const aggregated = new Map<string, { quarter: number; payorName: string; atcCode: string; gross: Decimal; cwt: Decimal }>()

    for (const cert of certificates) {
      const key = `${cert.quarter}|${cert.payorName}|${cert.atcCode}`
    ...
    return NextResponse.json({
      summary: Array.from(aggregated.values()).map((row) => ({
        ...row,
